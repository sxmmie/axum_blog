...
pub async fn get_players(State(pg): State<PgPool>) -> Result<(StatusCode, String), (StatusCode, String)> {
    let rows = sqlx::query_as::<_, PlayerRow>("SELECT player_id, name, age, wing FROM players ORDER BY player_id")
        .fetch_all(&pg)
        .await
...
pub async fn create_player(State(pg): State<PgPool>, Json(player): Json<CreatePlayerReq>) -> Result<(StatusCode, String), (StatusCode, String)> {
    let row = sqlx::query_as::<_, PlayerRow>("INSERT INTO players (name, age, wing) VALUES ($1, $2, $3) RETURNING player_id, name, age, wing")
        .bind(&player.name)
        .bind(player.age)
        .bind(player.wing)
        .fetch_one(&pg)
